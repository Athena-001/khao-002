...
class _MyhistoryState extends State<Myhistory> {
  ...
  void initState() {
    ...
    if (additivesString != null && additivesString.isNotEmpty) {
      additivesList = additivesString
          .split(',')
          .map((additive) => additive.trim())
          .toList();
    } else {
          ...
          child: Column(
            children: [
              ...
              Nutrition_panel(
                nutrition_data: {
                  "Carbs per 100gms":
                      widget.data?['carbs_100gms'].toString() ?? "not",
                  "Protein per 100gms":
                      widget.data?['protein_100gms'].toString() ?? "not",
                },
              ),
              J_veg_nveg_vegan(kyahai: 'vegan'),
              Additives(additives: additivesList),
              Allergents(aller: widget.data?['allergens']),
              Ingridients(ingri: widget.data?['ingredients']),
            ],
          ),
        ));
  }
